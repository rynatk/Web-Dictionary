...
////////////////////////////////////////////////////////////////////////////////
  $('.add-definition-form').on('submit', function (event) {
    ...
    (this).reset();  // adventure mode code lol
...
////////////////////////////////////////////////////////////////////////////////
  ...
  $('.word-search-form').on('submit', function (event) {
    ...
    $.ajax(API_ROOT + '/search', {                // .../search is the path on the website
      //method: 'GET', ... default
      data: { q:$('input[name=q]', this).val() }
      //data: $(this).serialize()                 ...preferred approach, not dependent upon html name
    }).done(function (data) {
      $('.word-list').empty();
      handleWordList(data);
    }).fail(handleAJAXError);
